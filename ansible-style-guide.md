...
Плейбук должен быть общим для всех стендов (dev, stage, preprod, prod) и быть готовым к запуску
на любом стенде.

Если часть плейбука не должна выполняться на определённом стенде, то в inventory этого стенда
