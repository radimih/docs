...
Аргументы описываются в **алфавитном** порядке.
...
Если роль не принимает никаких аргументов, то в `argument_specs` задаётся только точка входа в роль по-умолчанию `main`:
...
* Необходимо следить за тем, чтобы аргументы, объявленные как **обязательные** (`required: true`) не определялись в `defaults/main.yml`.
  Потому что Ansible возьмёт значение по-умолчанию и не среагирует на то, что аргумент фактически не был задан при вызове роли.
* К сожалению, нельзя расчитывать на доскональную проверку типов аргументов. Проверка производится путём попытки конвертации входного значения
  аргумента в объявленный тип. И например, в случае объявленного типа `str`, Ansible пропустит входное значение в виде списка. Потому что
  в Python список можно конвертировать в строку.
