...
* Необходимо следить за тем, чтобы аргументы, объявленные как **обязательные** (`required: true`) не определялись в `defaults/main.yml`.
  Иначе Ansible не будет выдать ошибку, когда такие аргументы не указываются при вызове роли.
  Иначе Ansible будет пропускать ситуацию, когда такой обязательный аргумент не указывается при вызове роли и возьмёт его значение из `defaults`.
