...
Если содержимое файла контируется Ansible'ом, то по возможности добавлять в него заголовок,
...
Примечания:
  * Это правило не касается [аргументов роли](#r1-аргументы-роли) в секции **role**, которые
...
* Для **необязательных** аргументов (`required: false`) по спецификации можно задать значение по-умолчанию (опция `default`),
  но надо иметь в виду, что фактическое значение по-умолчанию будет браться из `defaults/main.yml`.
