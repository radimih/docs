...
### f2. Отступы для списков

Списки всегда определяются с дополнительным отступом от родительского элемента.

* плохо:

  ```yaml
  - name: Общая настройка
    hosts: all
    roles:
...
Скаляр — это набор из нуля и более символов, представляющий в YAML одно неделимое значение
...
реальная необходимость (см. приложение [Когда скаляр в YAML требуется обрамлять в кавычки](#приложение-1-когда-скаляр-в-yaml-требуется-обрамлять-в-кавычки)).
...
В случае возникновения необходимости взять скаляр в кавычки (см. приложение [Когда скаляр в YAML требуется обрамлять в кавычки](#приложение-1-когда-скаляр-в-yaml-требуется-обрамлять-в-кавычки)) **двойные** кавычки `"` используются вместо одинарных `'` только в двух случаях:
...
* хорошо:
  ...
  name_v1: "{{ _default_name | default('Noname') }}"
  name_v2: "{{ _default_name | default(\"'Noname'\") }}"  # само значение по-умолчанию содержит одинарные кавычки
...
### s6. Короткая версия записи role используется только в исключительных случаях

Короткую версию записи **role** можно использовать только когда роль вызывается без [аргументов](#r1-аргументы-роли)
и длина строки не превышает 120 символов.
...
* плейбук:
  ...
  - name: Настройка frontend-серверов
    hosts: front_servers
    gather_facts: false
    roles:
    ...
    main:
      options:
        <имя_аргумента_1>:
          ...
          [elements: <тип элемента списка, если type == list>]
...
* К сожалению, нельзя рассчитывать на доскональную проверку типов аргументов. Проверка производится через попытку конвертировать входное значение
...
* `vars/main.yml`:
  ...
  ```yaml
  ---
  # Абсолютный путь к каталогу где хранятся PGP-ключи
...
Пример того, как ведёт себя роль при разных значениях этого параметра см. в приложении [Как параметр private_role_vars влияет на работу с ролями](#приложение-2-как-параметр-private_role_vars-влияет-на-работу-с-ролями).

## Приложения

### Приложение 1. Когда скаляр в YAML требуется обрамлять в кавычки
