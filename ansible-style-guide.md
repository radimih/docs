...
Скаляр — это [YAML-узел](https://yaml.org/spec/1.2.2/#3211-nodes), представляющий единичное значение — строку, число, логическое значение или null.

Для улучшения читабильности кода скаляры обрамляются в кавычки только если в этом возникает
реальная необходимость (см. ниже):
...
Обрамлять в **одинарные** кавычки (`'`) только когда:

* значение скаляра вступает в конфликт с синтаксисом YAML:
  ...
  * _содержит_ в случае [Flow Style Collection]() (пример):
    * символы
    * комбинации 
...
* значение скаляра должно однозначно интерпретироваться как строка, а не как число, логическое значение или null:

  ```yaml
  TODO: пример
  ```

Обрамлять в **двойные** кавычки (`"`) только когда значение скаляра содержит [escape-последовательности]:

  ```yaml
  TODO: пример
  ```

Более подробно про правила обрамления см. [Quoting](https://www.yaml.info/learn/quote.html).

#### s3. Директивы определяются только через map-синтаксис
...
#### s5. Короткая версия записи role используется только в ограниченных случаях
