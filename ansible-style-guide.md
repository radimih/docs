...
* хорошо:
  ...
  column_separator2: "\t"  # без кавычек или в одинарных кавычках переменная примет вместо знака табуляции
...
В случае возникновения необходимости взять скаляр в кавычи (см. приложение [Когда скаляр в YAML необходимо обрамлять в кавычки](#приложение-1-когда-скаляр-в-yaml-необходимо-обрамлять-в-кавычки)) **двойные** кавычки `"` вместо одинарных `'` используются только в двух случаях:

1. Если обрамляемая строка содержит [escape-последовательности](https://yaml.org/spec/1.2.2/#escaped-characters)
   (`\t`, `\x1B`, `\u2029` и т.д.).
3. Когда в обрамляемой строке используются **одинарные** кавычки.
