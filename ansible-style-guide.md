...
* `meta/main.yml` (у роли с тремя обязательными и двумя необязательными переменными):
  ```yaml
  ---
  ```
...
Таким образом, глядя в `meta/main.yml`, всегда видно **все** входные переменные роли
