...
реальная необходимость (см. [приложение]()).
...
#### Приложение 1. Правила использования кавычек для
...
* когда значение скаляра вступает в конфликт с синтаксисом YAML:
  ...
  * _содержит_ в себе символы `{` `}` `[` `]` `,` в случае использования JSON-подобного синтаксиса (`[ {x: 1, y: 2, z: 3} ]`)
    [Flow Collection Style](https://yaml.org/spec/1.2.2/#74-flow-collection-styles)

  ```yaml
  TODO: пример
  ```

* когда значение скаляра должно однозначно интерпретироваться как строка, а не как число, логическое значение или null:

  ```yaml
  TODO: пример
  ```

**Двойные** кавычки `"` используются когда необходимо использовать [escape-последовательности](https://yaml.org/spec/1.2.2/#escaped-characters):
...
Более подробно о правилах обрамления см. [Quoting](https://www.yaml.info/learn/quote.html).
