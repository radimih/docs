...
**Двойные** кавычки `"` используются только в двух случаях:

1. если обрамляемая строка должна содержать [escape-последовательности](https://yaml.org/spec/1.2.2/#escaped-characters)
   (`\t`, `\x1B`, `\u2029` и т.д.)
